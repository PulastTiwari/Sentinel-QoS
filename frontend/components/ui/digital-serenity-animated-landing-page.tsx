...
const DigitalSerenity = () => {
  ...
  useEffect(() => {
    const animateWords = () => {
      ...
      wordElements.forEach((word) => {
        const delay = parseInt((word as HTMLElement).getAttribute('data-delay') || '0') || 0;
        setTimeout(() => {
  ...
  useEffect(() => {
    const wordElements = document.querySelectorAll('.word-animate');
    const handleMouseEnter = (e: Event) => { if (e.target && e.target instanceof HTMLElement) e.target.style.textShadow = '0 0 20px rgba(203, 213, 225, 0.5)'; };
    const handleMouseLeave = (e: Event) => { if (e.target && e.target instanceof HTMLElement) e.target.style.textShadow = 'none'; };
    wordElements.forEach((word) => {
      word.addEventListener('mouseenter', handleMouseEnter as any);
      word.addEventListener('mouseleave', handleMouseLeave as any);
    });
    return () => {
      wordElements.forEach((word) => {
        if (word) {
          word.removeEventListener('mouseenter', handleMouseEnter as any);
          word.removeEventListener('mouseleave', handleMouseLeave as any);
        }
      });
    };
  }, []);
  ...
  useEffect(() => {
    ...
    floatingElementsRef.current = Array.from(elements) as any[];
    const handleScroll = () => {
      if (!scrolled) {
        setScrolled(true);
        ...
        <div className="relative z-10 min-h-screen flex flex-col justify-between items-center px-6 py-10 sm:px-8 sm:py-12 md:px-16 md:py-20">
          <div className="text-center">
            <h2 className="text-xs sm:text-sm font-mono font-light text-slate-300 uppercase tracking-[0.2em] opacity-80">
              ...
              <span className="word-animate" data-delay="300">2025 - AI Challenge</span>
            </h2>
